...
async function convertFilesToMarkdown(dir = DOCS_ROOT) {
  const entries = await readdir(dir, { withFileTypes: true });
  const changes = [];
  for (const entry of entries) {
    const absolute = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      changes.push(...await convertFilesToMarkdown(absolute));
      continue;
    }
    if (!entry.isFile() || entry.name.toLowerCase().endsWith(".md")) continue;
    const parsed = path.parse(entry.name);
    const target = await uniqueMdTarget(dir, parsed.name || entry.name);
    await rename(absolute, target);
    changes.push({
      from: path.relative(DOCS_ROOT, absolute).replace(/\\/g, "/"),
      to: path.relative(DOCS_ROOT, target).replace(/\\/g, "/"),
    });
  }
  return changes;
}
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/normalize-md" && req.method === "POST") {
    const changes = await convertFilesToMarkdown();
    await refreshCache(true);
